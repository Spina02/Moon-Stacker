...
def sharpen_with_edges(images, sobel_images, strength=1.5, k=1.5):
    sharpened_images = []
    for i, (image, sobel_mask) in enumerate(zip(images, sobel_images)):
...
def main():
    #config_init()
    images = read_images(config.input_folder)
    sobel images = sobel_images(images)
    canny_images(images)
