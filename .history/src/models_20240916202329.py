...
def sobel_images(images):
    sobel_images = []
    for image in images:
        sobel_image = np.zeros_like(image, dtype=np.float32)  # immagine sobelizzata vuota

        # Apply Sobel filter for each channel
        channels = cv2.split(image)
        for channel in range(image.shape[2]):  # loop su ogni canale (RGB o altri canali)
            channel = image[:, :, i]
            ...
            # Inserisci il canale sobelizzato nell'immagine finale
            sobel_image[:, :, i] = sobel

        # Normalize the result to 0-1 and append to the list
        sobel_images.append(normalize(sobel_image))
