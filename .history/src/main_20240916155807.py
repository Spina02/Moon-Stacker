...
def sobel_images(images):
    for i, image in enumerate(images):
        ...
        sobel = sobel.astype(np.uint16)

        save_image(sobel, f'./images/edge/sobel_{i}')
        return sobel

def sharpen_with_edges(images, edges, strength=1.5, k=1.5):
    sharpened_images = []
    for i, (image, edges_mask) in enumerate(zip(images, edges)):
        # Normalize Sobel mask to 0-1 range
        edges_mask = edges_mask.astype(np.float32) / 255.0
        #make the mask 3D
        ...
        final_image = edges_mask * high_boost + (1 - edges_mask) * image
        
        sharpened_images.append(final_image.astype(np.uint8))

    return edges_mask#sharpened_images

def main():
    #config_init()
    images = read_images(config.input_folder)
    #sobel_images = sobel_images(images)
    edges = canny_images(images)
    final_images = sharpen_with_edges(images, edges, strength=1.5, k=1.5)
