...
def unsharp_mask(images, model, strength):
    ...
    for image, blurred_image, sobel_mask in zip(images, blurred_images, normalized_sobel_masks):
        # make sobel mask 3 channels
        sobel_mask = cv2.merge([sobel_mask, sobel_mask, sobel_mask])
        # Adjust the strength based on the Sobel mask
        sharp_component = cv2.multiply(image.astype(np.float32), 1 + strength * sobel_mask.astype(np.float32))
        blurred_component = cv2.multiply(blurred_image.astype(np.float32), - strength * sobel_mask.astype(np.float32))
        if i == 0:
            save_image(to_16bit(sharp_component), './images/sharpened/sharp')
            save_image(to_16bit(blurred_component), './images/blurred/blur')
            i += 1
