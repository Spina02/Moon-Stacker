...
def perform_denoising(model, image, device):
    ...
    denoised_image = (image_denoised_rgb * 65535).astype(np.uint16)
...
def unsharp_mask(images, model, strength, k = 1.5):
    ...
    sharpened_images = high_boost(images, model, k = k)
...
def high_boost(images, model, k = 1.5):
    ...
    for i, image in enumerate(images):
        ...
        blurred_l = perform_denoising(model, channels[0], torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu'))

        mask = blurred_l - channels[0]
        sharpened_l = channels[0] + k * mask

        channels[0] = sharpened_l

        sharpened_image = cv2.merge(channels)
        image_sharpened_rgb = (color.lab2rgb(sharpened_image) * 65535).astype(np.uint16)

        images[i] = image_sharpened_rgb
