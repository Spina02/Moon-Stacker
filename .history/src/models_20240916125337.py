...
def high_boost(images, model, k = 1.5):
    # only l channel (lab)
    lab_images = [color.rgb2lab(image.astype(np.float32) / 65535.0) for image in images]
    channels = [list(cv2.split(lab_image)) for lab_image in lab_images]

    sharpened_images = dncnn_images(model, channels[0])
    masks = [image - sharpened for image, sharpened in zip(images, sharpened_images)]
    sharpened_images = [image + k* mask for image, mask in zip(images, masks)]

    # merge the channels
    sharpened_images = [cv2.merge(channels) for channels in sharpened_images]
