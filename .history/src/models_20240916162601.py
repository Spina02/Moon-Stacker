...
def sobel_images(images):
    ...
    for i, image in enumerate(images):
        ...
        #sobel = cv2.medianBlur(to_16bit(sobel), 3)

        # Normalize the result to 0-255
        sobel = to_16bit(sobel)
...
def unsharp_mask(images, model, strength, k=1.5):
    ...
    normalized_sobel_masks = [cv2.normalize(mask, None, 0, 1, cv2.NORM_MINMAX).astype(np.float32) for mask in sobel_masks]
    ...
    for image, blurred_image, sobel_mask in zip(images, blurred_images, normalized_sobel_masks):
        ...
        adjusted_strength = strength * sobel_mask
        merged_image = cv2.addWeighted(image, 1 + adjusted_strength, blurred_image, -adjusted_strength, 0)
