import config
from image import read_images, save_image, to_8bit
...
def sharpen_with_edges(images, canny_images, strength=1.5, k=1.5):
    sharpened_images = []
    for i, (image, sobel_mask) in enumerate(zip(images, canny_images)):
        # Normalize Sobel mask to 0-1 range
        sobel_mask = sobel_mask.astype(np.float32) / 255.0

        # Apply an unsharp mask
        blurred = cv2.GaussianBlur(image, (5, 5), 0)
        high_boost = image + k * (image - blurred)

        # Apply sharpening stronger in Sobel-detected areas
        sharpened_image = cv2.addWeighted(image, 1 + strength, blurred, -strength, 0)
        
        # Blend sharpened image with the high-boost one using the edge mask
        final_image = sobel_mask * high_boost + (1 - sobel_mask) * sharpened_image
        
        sharpened_images.append(final_image)
...
def main():
    #config_init()
    images = read_images(config.input_folder)
    #sobel_images = sobel_images(images)
    canny_images = canny_images(images)
    final_images = sharpen_with_edges(images, canny_images, strength=1.5, k=1.5)
    save_images(final_images, config.output_folder)
