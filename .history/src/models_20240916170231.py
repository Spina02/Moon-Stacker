...
def unsharp_mask(images, model, strength):
    ...
    for image, blurred_image, sobel_mask in zip(images, blurred_images, normalized_sobel_masks):
        ...
        sobel_mask = sobel_mask.astype(np.float32)
        ...
        if i == 0:
            save_image(sharp_component.astype(np.uint8), './images/sharpened/sharp')
            save_image(blurred_component.astype(np.uint8), './images/blurred/blur')
            i += 1
        merged_image = cv2.add(sharp_component, blurred_component).astype(np.uint16)
