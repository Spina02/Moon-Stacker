...
def align_images(images, algo='orb', nfeatures=500):
    #images = [equalize_histogram(image) for image in imgs]
...
# --------------- Preprocessing ----------------
def preprocess_images(images, algo='orb', nfeatures=500, margin = 10, denoise = False, sharpen = True, align = True, crop = False, sigma = 1.5, strength = 4, denoise_alg = 'DnCnn'):
    imgs = images.copy()
    if align:
        # Align the images
    ...
    if sharpen:
        ...
        model = model_init() if denoise_alg == 'DnCnn' else None
        imgs = unsharp_mask(images, sigma = sigma, strength = -0.5, model = model)
        save_image(imgs[0], './images/output/unsharp_mask.png')

    if denoise:
        # Denoise the images using the selected algorithm
        imgs = denoise_images(imgs, denoise_alg)
