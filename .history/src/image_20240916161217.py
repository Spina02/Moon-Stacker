...
def to_16bit(image):
    return cv2.normalize(image, None, 0, 65535, cv2.NORM_MINMAX).astype(np
...
def read_folder(folder_path):
    ...
    for i in range(min(MAX_IMG, len(folder))):
        image_paths.append(os.path.join(folder_path, folder[i]))
    return image_paths
