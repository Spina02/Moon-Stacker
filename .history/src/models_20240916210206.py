...
def unsharp_mask(images, model, strength):
    ...
    for image, blurred_image, sobel_mask in zip(images, blurred_images, sobel_masks):
        ...
        sharp_component = cv2.multiply(image, 1 + normalizestrength * sobel_mask)
        sharp_component = normalize(sharp_component)
        blurred_component = cv2.multiply(blurred_image, 1 - normalize(strength * sobel_mask))
        ...
        if i == 0:
            # normalize 0-255
            s_component = (sharp_component * 255).astype(np.uint8)
            b_component = (blurred_component * 255).astype(np.uint8)
