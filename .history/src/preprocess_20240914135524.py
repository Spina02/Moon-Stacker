...
def preprocess_images(images,           align       = True,
                      algo    = 'orb',  nfeatures   = 5000,
                      crop    = True,   margin      = 10,
                      denoise = False,  denoise_alg = 'DnCnn',
                      unsharp = True,   grayscale   = True):
    ...
    if unsharp:
        ...
        imgs = unsharp_mask(imgs, model_init())

    imgs = [cv2.cvtColor(image, cv2.COLOR_RGB2GRAY) for image in imgs] if grayscale else imgs
    #if True:#denoise:
