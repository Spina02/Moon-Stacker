...
def perform_denoising(model, image, device):
    ...
    if len(image.shape) == 3:
        channels = list(cv2.split(image))#cv2.cvtColor(image, cv2.COLOR_RGB2LAB)))
    else:
        channels = [image]
...
def unsharp_mask(images, model):
    blurred_images = dncnn_images(model, images)
    sharpened_images = [sharpen_image(image) for image in images]
    #sharpened_images = images
    merged_images = [cv2.addWeighted(sharpened_image, 0.93, blurred_image, 0.07, 0) for sharpened_image, blurred_image in zip(sharpened_images, blurred_images)]
