...
def unsharp_mask(images, sigma=1.5, strength=3, model=None):
    if model:
        ...
        blurred_images = [perform_denoising(model, image, device) for image in images]
        del device
        gc.collect()
    else:
        blurred_images = [cv2.GaussianBlur(image, (0, 0), sigma) for image in images]

    sharpened_images = [cv2.AddWeighted(image, 1 + strength, blurred_images, -strength) for image in images]
    images * (1 + strength) - blurred_images * strength
